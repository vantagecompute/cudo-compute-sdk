...
class CudoComputeSDK:
    ...
    def _build_create_vm_body(
        ...
        memory_gib: int,
        gpus: int,
        boot_disk_size_gib: Optional[int],
        password: Optional[str],
        ssh_key_source: Optional[str],
        custom_ssh_keys: Optional[List[str]],
        start_script: Optional[str],
        metadata: Optional[Dict[str, str]],
        security_group_ids: Optional[List[str]],
        storage_disk_ids: Optional[List[str]],
        commitment_term: Optional[str],
        expire_time: Optional[str],
        ttl: Optional[str],
        nics: Optional[List[Dict[str, Any]]],
        validate_only: bool,
    ) -> Dict[str, Any]:
        """Build the request body for VM creation.

        This is a helper method to reduce complexity in create_vm.

        Returns:
            Dictionary containing the request body for VM creation API
        """
        body: Dict[str, Any] = {
        ...
        optional_params = {
            "bootDiskSizeGib": boot_disk_size_gib,
            "password": password,
            "sshKeySource": ssh_key_source,
            "customSshKeys": custom_ssh_keys,
            "startScript": start_script,
            "metadata": metadata,
            "nics": nics,
            "storageDiskIds": storage_disk_ids,
            "commitmentTerm": commitment_term,
        ...
        # security_group_ids is only added if nics are not provided
    ...
    async def create_vm(
        ...
        Example:
            ...
            ... )
        """
        body = self._build_create_vm_body(
            vm_id=vm_id,
            ...
            storage_disk_ids=storage_disk_ids,
        ...
        if not response.is_success:
            try:
                ...
                logger.error(
                    f"VM creation failed with status {response.status_code}: {error_body}"
                )
            except Exception:
                logger.error(
                    f"VM creation failed with status {response.status_code}: {response.text}"
                )
    ...
    async def list_machine_types(
        ...
        Returns:
            List of MachineType model instances
        """
        params = {}
        if project_id:
            params["projectId"] = project_id
        ...
        response.raise_for_status()
        data = response.json()
        machine_types_data = data.get("machineTypes", [])
        return [MachineType(**mt) for mt in machine_types_data]
    ...
    async def get_machine_type(
        ...
        for mt in machine_types:
            if mt.data_center_id == data_center_id and mt.id == machine_type_id:
                return mt
